 ...
 * She typed a random string of length in the password field
 ...
 * + special_characters = "!@#$%^&*()-+" Input Format
 * 
...
public class StrongPassword {
	...
	static boolean hasLowerCaseChar(String s) {
		return s.matches(".*[a-z]+.*");
	}

	static boolean hasUpperCaseChar(String s) {
		return s.matches(".*[A-Z]+.*");
	}
    ...
    static int minimumNumber(int n, String password) {
    	...
    	if (password.length() < MIN_LENGTH - minimumNumber) {
    		minimumNumber = Math.abs(password.length() - (minimumNumber + (MIN_LENGTH - minimumNumber)));
    	}
